...
namespace Elsa.Persistence.Specifications.WorkflowInstances
{
    public class WorkflowSearchTermSpecification : Specification<WorkflowInstance>
    {
        ...
        public override Expression<Func<WorkflowInstance, bool>> ToExpression() => x => x.Name!.Contains(SearchTerm) || x.Id.Contains(SearchTerm) || x.ContextId!.Contains(SearchTerm) || x.CorrelationId!.Contains(SearchTerm);
    }
}